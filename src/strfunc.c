...
char *str_encoding(char *src, char *src_codepage, char *dst_codepage, BOOL bFree)
{
    ...
    if ( src_codepage == NULL )
        logWarning("Не определена исходная кодировка");
    else if ( dst_codepage == NULL )
        logWarning("Не определена результирующая кодировка");
        ...
        logWarning("Не обрабатываемая перекодировка <%s> -> <%s>", src_codepage, dst_codepage);
    return src;
}
...
BOOL isnumeric(const char *str)
{
    ...
    {
        // if (DBG_MODE) logInfo("WARNING. Empty string <%s>", str);
        return FALSE;
    }
    ...
    strtod(str, &p);
    BOOL result = (*p == '\0');
    // if (DBG_MODE) logInfo("Is NUM <%s> [%s] [%d]", str, p, result);
    ...
    if (result)
    {
        ...
        result = (!((i == 10) || (i == 12)));
        //if (DBG_MODE) logInfo("INN <%s> [%d] [%d]", str, i, result);
    }
...
int decimal_point(const char *number)
{
    ...
    for (i = last; i--; i >= 0)
    {
        // if (DBG_MODE) logInfo("Find decimal point <%c>", *(number+i));
        if (*(number+i) == '.')
            break;
        else
            count++;
...
char *strcopy(const char *str)
{
    ...
    if (copy)
        ...
        logInfo("Memory allocation error");
